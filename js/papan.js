...
// kontainerTombol.innerHTML = animateDadu
...
function Player(color, playerNumber) {
    ...
    this.drawPlayer = function () {
        ...
        if (this.position == 11) {
            //---- ladder 4 (11 - 27)
            if (this.playerNumber == urutan) {
                //---- pertanyaan hanya ditampilkan pd browser player ybs
                if (ambilSoal()) {
                    //---- Kalau jawaban benar maka naik tangga
                    this.position = 27;
                } else {
                    this.position += 1;
                }
            ...
            switch (this.playerNumber) {
                case 1: //---------update posisi naik tangga ---------
                    update(
                        ref(DBGame, `room/${roomJoin}/paraPemain/${arrKey[1]}`),
                        {
                ...
                case 2: //---------update posisi naik tangga---------
                    update(
                        ref(DBGame, `room/${roomJoin}/paraPemain/${arrKey[2]}`),
                        {
                            posisi: this.position,
                        }
                    );
                ...
                case 3: //---------update posisi naik tangga---------
                    update(
                        ref(DBGame, `room/${roomJoin}/paraPemain/${arrKey[3]}`),
                        {
                            posisi: this.position,
                        }
                    );
                ...
                case 4: //---------update posisi naik tangga---------
                    update(
                        ref(DBGame, `room/${roomJoin}/paraPemain/${arrKey[4]}`),
                        {
                            posisi: this.position,
                        }
                    );
        ...
        //---- Turun dipatuk Ular
        if (this.position == 45) {
            //---- snake 5 (kuning hitam) (45 - 17)
            if (this.playerNumber == urutan) {
                //---- pertanyaan hanya ditampilkan pd browser player ybs
                if (!ambilSoal()) {
                    //---- Kalau jawaban salah maka turun
                    this.position = 17;
                } else {
                    this.position += 1;
                }
...
(function () {
    ...
    get(qryParaPemain).then((qryParaPemainSnapshot) => {
        qryParaPemainSnapshot.forEach((childSnapShot) => {
            ...
            profilElement.innerHTML = `
               <div class="conProfil"> <i class="profilIcon Character_sprite grid-cell ${
                   childSnapShot.val().icon
               }" id="pemain${childSnapShot.val().urutan}"></i></div>
                <span id="nama${childSnapShot.val().urutan}">${
                childSnapShot.val().namaPemain
            }</span>
            `;
    ...
    let refeshUrutan = (
        ...
        switch (parUrutan) {
            case 1:
                ...
                if (
                    parAngka <= boxArr.length - 1 - player1.position &&
                    player1.isActive
                ) {
                    updates[`/${arrKey[parUrutan]}/posisi`] =
                        player1.position + parAngka; //update data posisi
                }
                break;
            case 2:
                if (!player2.isActive) {
                ...
                if (
                    parAngka <= boxArr.length - 1 - player2.position &&
                    player2.isActive
                ) {
                    updates[`/${arrKey[parUrutan]}/posisi`] =
                        player2.position + parAngka;
                }
                break;
            case 3:
                if (!player3.isActive) {
                ...
                if (
                    parAngka <= boxArr.length - 1 - player3.position &&
                    player3.isActive
                ) {
                    updates[`/${arrKey[parUrutan]}/posisi`] =
                        player3.position + parAngka;
                }
                break;
            case 4:
                if (!player4.isActive) {
                ...
                if (
                    parAngka <= boxArr.length - 1 - player4.position &&
                    player4.isActive
                ) {
                    updates[`/${arrKey[parUrutan]}/posisi`] =
                        player4.position + parAngka;
                }
    ...
    let bidakJalan = (parBidak) => {
        ...
        parBidak.forEach((angka, idx, arr) => {
            ...
            switch (arr[idx].urutan) {
                case 1:
                    player1.position = arr[idx].posisi;
                    player1.drawPlayer();
                    ...
                    break;
                case 2:
                    player2.position = arr[idx].posisi;
                    player2.drawPlayer();
                    ...
                    break;
                case 3:
                    player3.position = arr[idx].posisi;
                    player3.drawPlayer();
                    ...
                    break;
                case 4:
                    player4.position = arr[idx].posisi;
                    player4.drawPlayer();
                    if (player4.position == boxArr.length - 1) {
                        // update(ref(DBGame, `room/${roomJoin}}`), {winner: nama}); //update data pemenang
                        TheWinnner = arr[idx].namaPemain;
    ...
    btnKeluar.addEventListener("click", () => {
        onDisconnect(
            ref(DBGame, `room/${roomJoin}/paraPemain/` + idPemainJoin)
        ).remove();
        //goOffline(DBGame);
    ...
    onAuthStateChanged(auth, (pemain) => {
        if (pemain) {
            ...
            tombolElement.innerHTML = animateDadu;
            ...
            if (jawaban) {
                const audio = new Audio(
                    "http://topsoundeffect.com/sites/default/files/shake-dice-on-table-sound-effect-4147.mp3"
                );
                audio.play();
            }

            //         -- kalau disconnect akan remove pemain
            onDisconnect(idPemain).remove();
        }
    });
})();
